...
const resolvers = {
    ...
    Mutation: {
        ...
        addFixedExpense: async (_, { user_id, description, expenseAmount }, context) => {
            try {
                if (!context.user) {
                    throw new Error('You must be logged in to add a Fixed Expense');
                }
                ...
                user.fixedExpenses.push(newFixedExpense);
                await user.save();
